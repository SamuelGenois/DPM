...
public class ObstacleAvoidance extends Thread{
	...
	private final int bandwidth = 5;		//The maximum deviation from the nominal distance before adjustment
	...
	public void endTravel(){
		if (Math.abs((Repository.getX()-x_init)/a-(Repository.getY()-y_init)/b) > 4.0){
			if (!avoidanceStarted){
				Sound.beep();
			}
		...
		if (Math.abs((Repository.getX()-x_init)/a-(Repository.getY()-y_init)/b) < 2.0 && avoidanceStarted){
			avoiding = false;
			scanning = false;
			Sound.twoBeeps();
		}
